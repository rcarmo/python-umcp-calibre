...
class CalibreUmcpAction(InterfaceAction):
    ...
    def configure_bridge(self):
        ...
        import_staging_root = QLineEdit(str(prefs["import_staging_root"] or ""), dialog)
        import_staging_max_bytes = QSpinBox(dialog)
        import_staging_max_bytes.setRange(1, 1_073_741_824)
        import_staging_max_bytes.setValue(int(prefs["import_staging_max_bytes"] or 104857600))
        import_staging_ttl = QSpinBox(dialog)
        import_staging_ttl.setRange(60, 86400)
        import_staging_ttl.setValue(int(prefs["import_staging_ttl_seconds"] or 3600))
        ...
        form.addRow("Attachment staging root (optional)", import_staging_root)
        form.addRow("Attachment staging maximum bytes", import_staging_max_bytes)
        form.addRow("Attachment staging expiry (seconds)", import_staging_ttl)
        ...
        prefs["import_staging_root"] = import_staging_root.text().strip()
        prefs["import_staging_max_bytes"] = import_staging_max_bytes.value()
        prefs["import_staging_ttl_seconds"] = import_staging_ttl.value()
